...
struct MockManagedLockWatcher : public MockImageWatcher {
};
...
struct BaseRequest {
  ...
  static T* create(ManagedLock<MockManagedLockWatcher> *watcher,
...
struct AcquireRequest<MockManagedLockWatcher> :
  ...
  static AcquireRequest* create(ManagedLock<MockManagedLockWatcher> *managed_lock,
                                Context *on_finish) {
    return BaseRequest::create(managed_lock, on_finish);
  }
...
struct ReacquireRequest<MockManagedLockWatcher> :
  ...
  static ReacquireRequest* create(ManagedLock<MockManagedLockWatcher> *managed_lock,
...
template class librbd::ManagedLock<librbd::MockManagedLockWatcher>;
...
public:
  typedef ManagedLock<MockManagedLockWatcher> MockManagedLock;
  ...
  void expect_get_watch_handle(MockManagedLockWatcher *mock_watcher,
                               uint64_t watch_handle = 1234567890) {
    EXPECT_CALL(*mock_watcher, get_watch_handle())
      .WillOnce(Return(watch_handle));
  }

  void expect_watcher_is_registered(MockManagedLockWatcher *mock_watcher,
                                    bool is_registered = true) {
    EXPECT_CALL(*mock_watcher, is_registered())
      .WillOnce(Return(is_registered));
  }

  void expect_acquire_lock(MockManagedLock &mock_lock,
                           MockAcquireRequest &acquire_request, int r) {
    expect_watcher_is_registered(mock_lock.watcher());
    expect_get_watch_handle(mock_lock.watcher());
    EXPECT_CALL(acquire_request, send())
                  .WillOnce(QueueRequest(&acquire_request, r,
                            mock_lock.work_queue()));
  }

  void expect_release_lock(MockManagedLock &mock_lock,
  ...
  void expect_reacquire_lock(MockManagedLock &mock_lock,
                             MockReacquireRequest &mock_reacquire_request,
                             int r) {
    expect_get_watch_handle(mock_lock.watcher(), 98765);
...
TEST_F(TestMockManagedLock, StateTransitions) {
  ...
  expect_acquire_lock(managed_lock, request_lock_acquire2, 0);
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
  ASSERT_TRUE(is_lock_owner(managed_lock));

  MockReleaseRequest shutdown_release;
  expect_release_lock(managed_lock, shutdown_release, 0);
...
TEST_F(TestMockManagedLock, AcquireLockLockedState) {
  ...
  expect_acquire_lock(managed_lock, try_lock_acquire, 0);
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
...
TEST_F(TestMockManagedLock, AcquireLockAlreadyLocked) {
  ...
  MockAcquireRequest try_lock_acquire;
  expect_acquire_lock(managed_lock, try_lock_acquire, -EAGAIN);
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
  ASSERT_FALSE(is_lock_owner(managed_lock));
...
TEST_F(TestMockManagedLock, AcquireLockBusy) {
  ...
  MockAcquireRequest try_lock_acquire;
  expect_acquire_lock(managed_lock, try_lock_acquire, -EBUSY);
...
TEST_F(TestMockManagedLock, AcquireLockError) {
  ...
  MockAcquireRequest try_lock_acquire;
  expect_acquire_lock(managed_lock, try_lock_acquire, -EINVAL);
...
TEST_F(TestMockManagedLock, AcquireLockBlacklist) {
  ...
  expect_acquire_lock(managed_lock, request_lock_acquire, -EBLACKLISTED);
...
TEST_F(TestMockManagedLock, ReleaseLockError) {
  ...
  expect_release_lock(managed_lock, release, -EINVAL);
...
TEST_F(TestMockManagedLock, ConcurrentRequests) {
  ...
  expect_watcher_is_registered(&image_watcher);
  expect_get_watch_handle(&image_watcher);
  ...
  expect_acquire_lock(managed_lock, request_acquire, 0);
...
TEST_F(TestMockManagedLock, ReacquireLock) {
  ...
  MockAcquireRequest request_lock_acquire;
  expect_acquire_lock(managed_lock, request_lock_acquire, 0);
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
  ASSERT_TRUE(is_lock_owner(managed_lock));

  MockReacquireRequest mock_reacquire_request;
  C_SaferCond reacquire_ctx;
  expect_reacquire_lock(managed_lock, mock_reacquire_request, 0);
...
TEST_F(TestMockManagedLock, ReacquireLockError) {
  ...
  MockReacquireRequest mock_reacquire_request;
  C_SaferCond reacquire_ctx;
  expect_reacquire_lock(managed_lock, mock_reacquire_request, -EOPNOTSUPP);

  MockReleaseRequest reacquire_lock_release;
  expect_release_lock(managed_lock, reacquire_lock_release, 0);

  MockAcquireRequest reacquire_lock_acquire;
  expect_acquire_lock(managed_lock, reacquire_lock_acquire, 0);
