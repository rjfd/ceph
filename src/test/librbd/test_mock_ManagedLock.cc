...
struct BaseRequest {
  ...
  static T* create(librados::IoCtx& ioctx, Watcher *watcher, ContextWQ *work_queue,
...
public:
  ...
  void expect_reacquire_lock(MockWatcher& watcher,
                             ContextWQ *work_queue,
  ...
  void expect_flush_notifies(MockWatcher *mock_watcher) {
...
TEST_F(TestMockManagedLock, StateTransitions) {
  ...
  MockWatcher image_watcher(mock_image_ctx.md_ctx);
  ...
  expect_acquire_lock(image_watcher, ictx->op_work_queue, request_lock_acquire1, 0);
  ...
  expect_acquire_lock(image_watcher, ictx->op_work_queue, request_lock_acquire2, 0);
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
  ASSERT_TRUE(is_lock_owner(managed_lock));
...
TEST_F(TestMockManagedLock, AcquireLockLockedState) {
  ...
  expect_acquire_lock(image_watcher, ictx->op_work_queue, try_lock_acquire, 0);
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
...
TEST_F(TestMockManagedLock, AcquireLockAlreadyLocked) {
  ...
  MockAcquireRequest try_lock_acquire;
  expect_acquire_lock(image_watcher, ictx->op_work_queue, try_lock_acquire, -EAGAIN);
...
TEST_F(TestMockManagedLock, AcquireLockBusy) {
  ...
  MockAcquireRequest try_lock_acquire;
  expect_acquire_lock(image_watcher, ictx->op_work_queue, try_lock_acquire, -EBUSY);
...
TEST_F(TestMockManagedLock, AcquireLockError) {
  ...
  MockAcquireRequest try_lock_acquire;
  expect_acquire_lock(image_watcher, ictx->op_work_queue, try_lock_acquire, -EINVAL);
...
TEST_F(TestMockManagedLock, AcquireLockBlacklist) {
  ...
  expect_acquire_lock(image_watcher, ictx->op_work_queue, request_lock_acquire, -EBLACKLISTED);
...
TEST_F(TestMockManagedLock, ConcurrentRequests) {
  ...
  expect_acquire_lock(image_watcher, ictx->op_work_queue, request_acquire, 0);
...
TEST_F(TestMockManagedLock, ReacquireLock) {
  ...
  MockAcquireRequest request_lock_acquire;
  expect_acquire_lock(image_watcher, ictx->op_work_queue, request_lock_acquire, 0);
  ASSERT_EQ(0, when_acquire_lock(managed_lock));
  ASSERT_TRUE(is_lock_owner(managed_lock));

  MockReacquireRequest mock_reacquire_request;
  C_SaferCond reacquire_ctx;
  expect_reacquire_lock(image_watcher, ictx->op_work_queue, mock_reacquire_request, 0);
...
TEST_F(TestMockManagedLock, ReacquireLockError) {
  ...
  MockReacquireRequest mock_reacquire_request;
  C_SaferCond reacquire_ctx;
  expect_reacquire_lock(image_watcher, ictx->op_work_queue, mock_reacquire_request, -EOPNOTSUPP);
  ...
  expect_acquire_lock(image_watcher, ictx->op_work_queue, reacquire_lock_acquire, 0);
