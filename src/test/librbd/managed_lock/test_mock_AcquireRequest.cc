...
template class librbd::managed_lock::AcquireRequest<librbd::MockImageWatcher>;
...
public:
  typedef AcquireRequest<MockImageWatcher> MockAcquireRequest;
...
TEST_F(TestMockManagedLockAcquireRequest, Success) {
  ...
  C_SaferCond ctx;
  MockAcquireRequest *req = MockAcquireRequest::create(&managed_lock, &ctx);
  req->send();
