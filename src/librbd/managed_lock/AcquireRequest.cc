...
template <typename W>
AcquireRequest<W>* AcquireRequest<W>::create(ManagedLock<W> *managed_lock,
                                             Context *on_finish) {
    return new AcquireRequest(managed_lock, on_finish);
}

template <typename W>
AcquireRequest<W>::AcquireRequest(ManagedLock<W> *managed_lock,
                                  Context *on_finish)
  : m_ioctx(managed_lock->io_ctx()), m_watcher(managed_lock->watcher()),
    m_cct(reinterpret_cast<CephContext *>(m_ioctx.cct())),
    m_work_queue(managed_lock->work_queue()), m_oid(managed_lock->oid()),
    m_cookie(managed_lock->cookie()),
...
template <typename W>
AcquireRequest<W>::~AcquireRequest() {
}

template <typename W>
void AcquireRequest<W>::send() {
  send_lock();
}

template <typename W>
void AcquireRequest<W>::send_lock() {
  ldout(m_cct, 10) << __func__ << dendl;

  librados::ObjectWriteOperation op;
  rados::cls::lock::lock(&op, RBD_LOCK_NAME, LOCK_EXCLUSIVE, m_cookie,
                         ManagedLock<W>::WATCHER_LOCK_TAG, "", utime_t(), 0);
...
template <typename W>
void AcquireRequest<W>::handle_lock(int r) {
...
template <typename W>
void AcquireRequest<W>::send_get_lockers() {
  ldout(m_cct, 10) << __func__ << dendl;
...
template <typename W>
void AcquireRequest<W>::handle_get_lockers(int r) {
  ldout(m_cct, 10) << __func__ << ": r=" << r << dendl;
  ...
  if (lock_tag != ManagedLock<W>::WATCHER_LOCK_TAG) {
  ...
  if (!ManagedLock<W>::decode_lock_cookie(iter->first.cookie, &m_locker_handle)) {
...
template <typename W>
void AcquireRequest<W>::send_get_watchers() {
  ldout(m_cct, 10) << __func__ << dendl;
...
template <typename W>
void AcquireRequest<W>::handle_get_watchers(int r) {
  ldout(m_cct, 10) << __func__ << ": r=" << r << dendl;
...
template <typename W>
void AcquireRequest<W>::send_blacklist() {
...
template <typename W>
void AcquireRequest<W>::handle_blacklist(int r) {
  ldout(m_cct, 10) << __func__ << ": r=" << r << dendl;
...
template <typename W>
void AcquireRequest<W>::send_break_lock() {
  ldout(m_cct, 10) << __func__ << dendl;
...
template <typename W>
void AcquireRequest<W>::handle_break_lock(int r) {
  ldout(m_cct, 10) << __func__ << ": r=" << r << dendl;
...
template class librbd::managed_lock::AcquireRequest<
