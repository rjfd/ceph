...
template <typename I>
void ManagedLock<I>::acquire_lock(Context *on_acquired, bool try_lock) {
  int r = 0;
  {
    Mutex::Locker locker(m_lock);
    if (is_shutdown_locked()) {
      ...
      ldout(m_cct, 10) << dendl;
      execute_action(try_lock ? ACTION_TRY_LOCK : ACTION_ACQUIRE_LOCK,
                     on_acquired);
      return;
    }
  }
...
void ManagedLock<I>::send_acquire_lock() {
  ...
  pre_acquire_lock_handler(new FunctionContext([this](int r) {
...
void ManagedLock<I>::handle_acquire_lock(int r) {
  ...
  post_acquire_lock_handler(r, new FunctionContext([this, next_state](int r) {
    assert(m_lock.is_locked());

    if (r < 0 && next_state == STATE_LOCKED) {
      // release_lock without calling pre and post callbacks
      ...
      ReleaseRequest<I>* req = ReleaseRequest<I>::create(m_ioctx, m_watcher,
          ...
          new FunctionContext([this, r](int ret) {
            ...
            if (ret >= 0) {
              ...
              complete_active_action(STATE_LOCKED, ret);
            }
          }));
...
void ManagedLock<I>::send_release_lock() {
  ...
  pre_release_lock_handler(false, new FunctionContext([this](int r) {
    using managed_lock::ReleaseRequest;
    ReleaseRequest<I>* req = ReleaseRequest<I>::create(m_ioctx, m_watcher,
        m_work_queue, m_oid, m_cookie,
        util::create_context_callback<
...
void ManagedLock<I>::handle_release_lock(int r) {
  ...
  post_release_lock_handler(false, r, new FunctionContext([this](int r) {
...
void ManagedLock<I>::send_shutdown() {
  ...
  if (m_state == STATE_UNLOCKED) {
    ...
    m_work_queue->queue(new FunctionContext([this](int r) {
      shutdown_handler(r, new FunctionContext([this](int r) {
        complete_shutdown(r);
      }));
    }));
    return;
...
void ManagedLock<I>::send_shutdown_release() {
  ...
  pre_release_lock_handler(true, new FunctionContext([this, cookie](int r) {
    ldout(m_cct, 10) << ": r=" << r << dendl;
    using managed_lock::ReleaseRequest;
    ReleaseRequest<I>* req = ReleaseRequest<I>::create(m_ioctx, m_watcher,
        ...
        new FunctionContext([this](int r) {
          ...
          post_release_lock_handler(true, r,
              new FunctionContext([this](int r) {
                complete_shutdown(r);
              }));
        }));
