...
* ``namespace`` is a string that can be used to group tasks. For instance
  ...
  namespace, or similarly ``"rbd/remove"`` for RBD image removal tasks.
...
The pair ``(namespace, metadata)`` should unequivocally identify the task being
...
``TaskManager.run``::
  ...
  TaskManager.run(namespace, metadata, func, args=None, kwargs=None,
                  executor=None)
...
``TaskManager.run``, is the ``ThreadedExecutor``. You can check it's
