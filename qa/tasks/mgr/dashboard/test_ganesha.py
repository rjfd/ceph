...
class GaneshaTest(DashboardTestCase):
    ...
    def setUpClass(cls):
        ...
        cls._rados_cmd(['-p', 'ganesha', '-N', 'ganesha', 'create', 'conf-node1'])
        cls._rados_cmd(['-p', 'ganesha', '-N', 'ganesha', 'create', 'conf-node2'])
        cls._rados_cmd(['-p', 'ganesha', '-N', 'ganesha', 'create', 'conf-node3'])
        cls._ceph_cmd(['dashboard', 'set-ganesha-rados-pool-namespace', 'ganesha/ganesha'])
    ...
    def test_list_daemons(self):
        ...
        daemons = [d['daemon_id'] for d in daemons]
        self.assertIn('node1', daemons)
        self.assertIn('node2', daemons)
        self.assertIn('node3', daemons)

    @classmethod
    def create_export(cls, path, daemons, fsal):
        if fsal == 'CEPH':
    ...
    def tearDown(self):
        ...
        for exp in exports:
            self._task_delete("/api/nfs-ganesha/export/{}".format(exp['export_id']))

    def test_create_export(self):
        exports = self._get("/api/nfs-ganesha/export")
        self.assertEqual(len(exports), 0)

        data = self.create_export("/foo", ['node1', 'node2'], 'CEPH')
    ...
    def test_update_export(self):
        ...
        data = self._task_put('/api/nfs-ganesha/export/{}'.format(export['export_id']), export)
        exports = self._get("/api/nfs-ganesha/export")
        self.assertEqual(len(exports), 1)
        self.assertDictEqual(exports[0], data)
    ...
    def test_delete_export(self):
        export = self.test_create_export()
        self._task_delete("/api/nfs-ganesha/export/{}".format(export['export_id']))
    ...
    def test_get_export(self):
        ...
        data1 = self.create_export("/foo", ['node1', 'node2'], 'CEPH')
        data2 = self.create_export("mybucket", ['node2', 'node3'], 'RGW')

        export1 = self._get("/api/nfs-ganesha/export/1")
        self.assertDictEqual(export1, data1)

        export2 = self._get("/api/nfs-ganesha/export/2")
